...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {                                               //Avoids FSL
...
function updatePositions() {
  ...
  var scrollTop=document.body.scrollTop / 1250;                                               //Avoids FSL
  var items = document.getElementsByClassName('mover');
  var n=0;
  for (var i = 0; i < items.length; i++) {
    var phase = Math.sin(scrollTop+n);
    var x=items[i].basicLeft + 100 * phase-966.5+"px";
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  for (var i = 0; i < 32; i++) {                                    //only 32 needed
      ...
      elem.style.top = (Math.floor(i / 8) * s) + 'px';
